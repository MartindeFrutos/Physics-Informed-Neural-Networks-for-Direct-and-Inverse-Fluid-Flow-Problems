...
import pandas as pd 
writer = SummaryWriter('C:/Users/marti/Desktop/MUMAC/TFM/1 Dim Flow with Area Variation/Direct/logs')    
matplotlib.use("pgf")
...
class PINN_1DFlowA(torch.nn.Module):
    ...
    def Area_variation(self,X):
        ...
        AL=self.A0*3 # Final value of the area 
        k=2 # related with the curvature 
        return self.A0+(AL-self.A0)/(1+torch.exp(-k*(X-self.Ix[1]*0.5-self.Ix[0]*0.5)))
...
plt.legend()
plt.tight_layout()
plt.savefig(folder+'Variables distribution.pgf',dpi=1000)
plt.close()   
...
plt.ylabel(r'$A(x)$')
with torch.no_grad():
    plt.plot(X,N.Area_variation(X),color='black')
    plt.plot(X,X)
plt.savefig(folder+'Area distribution.pgf',dpi=1000)
plt.close()